...
class optmizer():
    ...
    def evaluation(self, individual):
        ...
        Grid = self.Grid()
        Grid.cells_new = list(Grid.cells_old)
        Grid.chem_new = list(Grid.chem_old)
        ...
        for step in range(1, Grid.max_steps):
            for row in range(0, Grid.size[0]):
                for col in range(0, Grid.size[1]):
                    ...
                    for chem in range(0,3):
                        #Chem evaporation
                        Grid.chem_new[row][col][chem] = 0.5* float(Grid.chem_old[row][col][chem])
                        #Chem diffusion
                        for j in range(-1,2):
                            for k in range(-1,2):
                                if (row+j >= 0) and (row+j < Grid.size[0]) and (col+k >= 0) and (col+k < Grid.size[1]):
                                    Grid.chem_new[row][col][chem]+=(1.0/16.0) * float(Grid.chem_old[row+j][col+k][chem])
                        #Chem production
                        if Grid.cells_old[row][col] == 1:
                            if self.target[row,col] == 1:
                                Grid.chem_new[row][col][chem]+=float(individual[16+chem])
                            else:
                                Grid.chem_new[row][col][chem]+=float(individual[19+chem])
                    #Growth And Death
                    ...
                    if step > (Grid.max_steps-5):
                        squaredError.append((int(self.target[row,col]) - int(Grid.cells_old[row][col]))**2)

            Grid.chem_old = list(Grid.chem_new)
            Grid.cells_old = list(Grid.cells_new)
            if self.save:
                Steps[step] = list(Grid.cells_old)
