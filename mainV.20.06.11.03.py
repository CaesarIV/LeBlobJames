...
class optmizer():
        ...
        def __init__ (self,):
            ...
            for row in range(0,self.size[0]):
                ...
                for col in range(0,self.size[1]):
                    self.cells["new"][row][col]=0
                    self.cells["old"][row][col]=0
            ...
            #seeds
            self.cells["old"][10][10]=1
            self.cells["new"] = self.cells["old"].copy()
            #seedsNum = 10
    ...
    def evaluation(self, individual):
        ...
        if self.save == True:
            Steps = {}
            cells=[]
            for row in range(0, len(Grid.cells["old"])):
                cells.append([])
                for col in range(0,len(Grid.cells["old"][row])):
                    cells[row].append(Grid.cells["old"][row][col])
            Steps[0]=cells
        ...
        for step in range(1,max_steps):
            ...
            for row in range(0, Grid.size[0]):
                ...
                for col in range(0, Grid.size[1]):
                    ...
                    for i in range(0,3):
                        ...
                        for j in range(-1,2):
                            for k in range(-1,2):
                                if (row+j >= 0) and (row+j < Grid.size[0]) \
                                   and (col+k >= 0) and (col+k < Grid.size[1]):
                                    Grid.chem["new"][row][col][i] += (1.0/16.0)*Grid.chem["old"][row+j][col+k][i]
                                    if Grid.cells["old"][row+j][col+k] == 1:
                    ...
                    #GrowthAndDeath
                    if neighbour_exist: #and Grow.check(x) == 1 and (random.random()>=0.5):
                        Grid.cells["new"][row][col] = 1
                    if Die.check(x) == 1:
                        Grid.cells["new"][row][col] = 0
                    if self.save:
                        cells[row].append(Grid.cells["new"][row][col])
            Grid.chem["old"] = Grid.chem["new"].copy()
            Grid.cells["old"] = Grid.cells["new"].copy()
            ...
            if step > max_steps-5:
                for row in range(0,Grid.size[0]):
                    for col in range(0,Grid.size[1]):
                        se.append((self.target[row,col]-Grid.cells["old"][row][col])**2)
        if self.save:
